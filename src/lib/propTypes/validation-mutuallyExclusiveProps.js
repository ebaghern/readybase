...
export default function mutuallyExclusiveOfType(propType, ...exclusiveProps) {
  ...
  if (exclusiveProps.length < 1) {
    throw new TypeError(
      'at least one prop that is mutually exclusive with this propType is required'
    );
  }

  const propList = exclusiveProps.join(', or ');

  const map = exclusiveProps.reduce(
    (acc, prop) => ({ ...acc, [prop]: true }),
    {}
  );
  const countProps = (count, prop) => count + (map[prop] ? 1 : 0);

  const validator = function mutuallyExclusiveProps(
    props,
    propName,
    componentName,
    ...rest
    ...
    if (exclusivePropCount > 1) {
      return new Error(
        `A ${componentName} cannot have more than one of these props: ${propList}`
      );
    }
  ...
  return wrapValidator(
    validator,
    `mutuallyExclusiveProps:${propList}`,
    exclusiveProps
  );
}
